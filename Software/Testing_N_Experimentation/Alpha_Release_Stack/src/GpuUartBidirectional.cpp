...
#include "esp_log.h"
#include <cstring>
#include "../include/GpuUartBidirectional.h"

using namespace arcos::communication;

static const char* TAG = "GPU_UART";

GpuUartBidirectional::GpuUartBidirectional()
  : initialized_(false)
  , frame_counter_(0)
  , last_frame_time_(0)
{
...
bool GpuUartBidirectional::init(int baud_rate){
  ESP_LOGI(TAG, "========================================");
  ESP_LOGI(TAG, "GPU: High-Throughput UART System");
  ESP_LOGI(TAG, "========================================");
  ESP_LOGI(TAG, "TX: %d bytes (1568 bits) @ 60Hz", GPU_SEND_BYTES);
  ESP_LOGI(TAG, "RX: %d bytes (316 bits) @ 60Hz", GPU_RECV_BYTES);
  ESP_LOGI(TAG, "========================================");
  ...
  esp_err_t err = uart_driver_install(GPU_UART_NUM, 2048, 2048, 0, NULL, 0);
  if(err != ESP_OK){
...
bool GpuUartBidirectional::sendPacket(MessageType type, const uint8_t* payload, uint8_t length){
...
bool GpuUartBidirectional::receivePacket(UartPacket& packet){
  if(!initialized_){
    return false;
  }
...
int GpuUartBidirectional::available(){
  size_t available_bytes = 0;
  uart_get_buffered_data_len(GPU_UART_NUM, &available_bytes);
  return available_bytes;
}

bool GpuUartBidirectional::sendPing(){
  uint8_t ping_data = 0xCD;
  return sendPacket(MessageType::PING, &ping_data, 1);
}

bool GpuUartBidirectional::sendAck(uint8_t ack_data){
  return sendPacket(MessageType::ACK, &ack_data, 1);
}

bool GpuUartBidirectional::sendDataFrame(){
...
void GpuUartBidirectional::printAnalytics(){
  uint32_t current_time = xTaskGetTickCount() * portTICK_PERIOD_MS;
  uint32_t elapsed = current_time - analytics_.start_time;
  ...
  ESP_LOGI(TAG, "========================================");
  ESP_LOGI(TAG, "        GPU ANALYTICS REPORT");
  ESP_LOGI(TAG, "========================================");
  ESP_LOGI(TAG, "Runtime:       %.1f sec", elapsed_sec);
  ESP_LOGI(TAG, "Frames Sent:   %u (%.1f fps)", analytics_.frames_sent, send_fps);
  ESP_LOGI(TAG, "Frames Recv:   %u (%.1f fps)", analytics_.frames_received, recv_fps);
  ESP_LOGI(TAG, "Packet Loss:   %u (%.2f%%)", analytics_.packets_lost, packet_loss);
  ESP_LOGI(TAG, "Checksum Err:  %u", analytics_.checksum_errors);
  ESP_LOGI(TAG, "Timeout Err:   %u", analytics_.timeout_errors);
  ESP_LOGI(TAG, "TX Throughput: %.2f kbps", send_kbps);
  ESP_LOGI(TAG, "RX Throughput: %.2f kbps", recv_kbps);
  
  ESP_LOGI(TAG, "TX Progress:   %u/60", analytics_.frames_sent % 60);
  ESP_LOGI(TAG, "RX Progress:   %u/60", analytics_.frames_received % 60);
  ESP_LOGI(TAG, "========================================");
  
  analytics_.last_report_time = current_time;
}

void GpuUartBidirectional::update(){
  UartPacket packet;
...
void GpuUartBidirectional::handleReceivedPacket(const UartPacket& packet){
